...
{ 
    ...
    for(char c: raw_string)
    {
        switch (c) {
            case 'A':
              result.insert(result.end(), {0,0,0,1});break;
            case 'U':
              result.insert(result.end(), {0,0,1,0});break;
            case 'C':
              result.insert(result.end(), {0,1,0,0});break;
            case 'G':
              result.insert(result.end(), {1,0,0,0});break;
            case 'a':
              result.insert(result.end(), {0,0,0,1});break;
            case 't':
              result.insert(result.end(), {0,0,1,0});break;
...
{ // (A)denine represented by    -1
  ...
  while (input.get(c)) {
    ...
    switch (c) {
    case 'A':
      buffered_output<int, float>(-1, buffer_idx, buffer_size, buffer, output);
      break;
    case 'a':
      buffered_output<int, float>(-1, buffer_idx, buffer_size, buffer, output);
      break;
    ...
    case 't':
      ...
      break;
    case 'C':
      buffered_output<int, float>(-2, buffer_idx, buffer_size, buffer, output);
      break;
    case 'c':
      buffered_output<int, float>(-2, buffer_idx, buffer_size, buffer, output);
      break;
    ...
    default: // Intend to cut the hard mask because it provide no information.
      if (!cleave_zero)
        buffered_output<int, float>(0, buffer_idx, buffer_size, buffer, output);
      break;
    }
  }
                          ...
                          std::vector<std::vector<int>> &result)
{
  ...
  while(std::getline(score_file,score_string))
  {
    ...
    if(this_score < 0.3) result[idx++] = {0,0,0,1};
    if(this_score >= 0.3 && this_score < 0.5)result[idx++] = {0,0,1,0};
    if(this_score >= 0.5 && this_score < 0.7)result[idx++] = {0,1,0,0};
    if(this_score >= 0.7)result[idx++] = {1,0,0,0};
  }
